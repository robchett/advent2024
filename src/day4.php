    ...
    #[\Override] public function task1(): TaskOutput
    {
        ...
        for($y = 0; $y < $this->input->sizeY; $y++) {
            for($x = 0; $x < $this->input->sizeX; $x++) {
                foreach ($cardinals as [$dx, $dy]) {
                    if (
                        $y + 3 * $dy >= $this->input->sizeY ||
                        $y + 3 * $dy < 0 ||
                        $x + 3 * $dx >= $this->input->sizeX ||
                        $x + 3 * $dx < 0 
                    ) {
                         continue ;
                    }
                    if (
                        $this->input[$y][$x] == 'X' &&
                        $this->input[$y + 1 * $dy][$x + 1 * $dx] == 'M' &&
                        $this->input[$y + 2 * $dy][$x + 2 * $dx] == 'A' &&
                        $this->input[$y + 3 * $dy][$x + 3 * $dx] == 'S'
                    ) {
                        $xmass++;
                    }
                }
        ...
        return new TaskOutput($xmass);
    }

    #[\Override] public function task2(): TaskOutput
    {
        $xmass = 0;
        for($y = 1; $y < $this->input->sizeY - 1; $y++) {
            for($x = 1; $x < $this->input->sizeX - 1; $x++) {
                if ($this->input[$y][$x] != 'A') {
                    continue;
                }
                if (
                    ([$this->input[$y - 1][$x - 1], $this->input[$y + 1][$x + 1]] == ['M', 'S'] || [$this->input[$y - 1][$x - 1], $this->input[$y + 1][$x + 1]] == ['S', 'M']) &&
                    ([$this->input[$y + 1][$x - 1], $this->input[$y - 1][$x + 1]] == ['M', 'S'] || [$this->input[$y + 1][$x - 1], $this->input[$y - 1][$x + 1]] == ['S', 'M'])
                ) {
                    $xmass++;
                }
