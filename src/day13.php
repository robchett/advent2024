...
return new class($input) implements Task {
    ...
    #[\Override] function __construct(string $input)
    {
        ...
        $equations = [];
        while (count($lines) >= 3) {
            [$l1, $l2, $l3, $_] = array_splice($lines, 0, 4);
            preg_match('/Button A: X\+(\d+), Y\+(\d+)/', $l1, $m1);
            preg_match('/Button B: X\+(\d+), Y\+(\d+)/', $l2, $m2);
            preg_match('/Prize: X=(\d+), Y=(\d+)/', $l3, $m3);
    ...
    {
        $coins = 0;
        foreach($this->equations as ['A' => $a, 'B' => $b, 'P' => $p]) {
            $A = (($b[1] * $p[0]) - ($b[0] * $p[1])) / (($b[1] * $a[0]) - ($b[0] * $a[1]));
            $B = ($p[0] - ($a[0] * $A)) / $b[0];
            if (is_int($A) && is_int($B)) {
                $coins += (3 * $A) + (1 * $B);
            }
        }
        return new TaskOutput($coins);
    }
