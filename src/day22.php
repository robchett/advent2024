...
return new class($input) implements Task {
    ...
    public array $secretNumbers;

    #[\Override] function __construct(string $input)
    {
        $this->secretNumbers = array_map(fn($i) => (int) $i, explode("\n", $input));
    }

    #[\Override] public function task1(): TaskOutput
    {
        $total = 0;
        foreach ($this->secretNumbers as $n0) {
            for ($i = 0; $i < 2000; $i++) {
                $n1 = (($n0 << 6) ^ $n0) % 16777216;
                $n2 = (($n1 >> 5) ^ $n1) % 16777216;
                $n3 = (($n2 << 11) ^ $n2) % 16777216;
                $n0 = $n3;
            }
            $total += $n0;
        }
    ...
    {
        $sequences = [];
        foreach ($this->secretNumbers as $n) {
            $n0 = $n;
            $sequence = [];
            $seenSequences = [];
            for ($i = 0; $i < 2000; $i++) {
                ...
                $n2 = (($n1 >> 5) ^ $n1) % 16777216;
                $n3 = (($n2 << 11) ^ $n2) % 16777216;
                $sequence[] = ($n3 % 10) - ($n0 % 10); 
                if ($i >= 3) {
                    if (!isset($seenSequences[implode(",", $sequence)])) {
                        $seenSequences[implode(",", $sequence)] = true;
                        $sequences[implode(",", $sequence)] ??= 0;
                        $sequences[implode(",", $sequence)] += ($n3 % 10);
                    }
        ...
        //asort($sequences);
        //print_r($sequences);
        return new TaskOutput(max($sequences));
    }
};