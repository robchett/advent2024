...
return new class($input) implements Task {
    ...
    {
        [$r1, $r2, $r3,, $commands] = explode("\n", $input);
        $this->r1 = (int) substr(trim($r1), 12);
        $this->r2 = (int) substr(trim($r2), 12);
        $this->r3 = (int) substr(trim($r3), 12);
        $this->commands = explode(",", substr(trim($commands), 9));
    }

    public function compute($r1, $r2, $r3, $commands, $part2): string 
    {
        ...
        while($ip < $limit) {
            $inId = (int) $commands[$ip] % 8;
            $op = (int) $commands[$ip + 1] % 8;
            $comboOp = match ($op) {
                0 => 0,
                1 => 1,
                2 => 2,
    ...
    {        
        return new TaskOutput($this->compute($this->r1, $this->r2, $this->r3, $this->commands, false));
    }
