...
return new class($input) implements Task {
    ...
    #[\Override] public function task1(): TaskOutput
    {
        ...
        foreach ($this->lists as $list) {
            $safe += $this->test($list)[0];
        }
        return new TaskOutput($safe);
    }
    ...
    #[\Override] public function task2(): TaskOutput
    {
        $safe = 0;
        foreach ($this->lists as $list) {   
            [$ok, $error] = $this->test($list);    
            if ($ok) {
                $safe++;
                continue;
            }
            foreach ([$error + 1, $error, $error - 1] as $k => $offset) {
                $new = $list;
                unset($new[$offset]);
                if ($this->test(array_values($new))[0]) {
                    $safe++;
                    continue 2;
                }                
            }
    ...
    protected function test(array $list): array 
    {
        $dir = $list[0] < $list[1] ? 'asc' : 'desc';
        for ($i = 1; $i < count($list); $i++) {
            $diff = match($dir) {
              'asc' =>  $list[$i] - $list[$i - 1],
              'desc' =>  $list[$i - 1] - $list[$i],
            };
            if ($diff > 3 || $diff < 1) 
            {
                return [false, $i - 1];
